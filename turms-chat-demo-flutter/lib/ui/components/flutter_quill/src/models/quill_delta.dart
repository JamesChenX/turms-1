...
class Operation {
  Operation._(
      this.key, this.length, this.data, Map<String, dynamic>? attributes)
  ...
  static Operation fromJson(Map data, {DataDecoder? dataDecoder}) {
    ...
    if (map.containsKey(Operation.insertKey)) {
      final data = dataDecoder(map[Operation.insertKey] as Object);
      final dataLength = data is String ? data.length : 1;
      return Operation._(Operation.insertKey, dataLength, data,
          map[Operation.attributesKey] as Map<String, dynamic>?);
    } else if (map.containsKey(Operation.deleteKey)) {
      final length = map[Operation.deleteKey] as int?;
      return Operation._(Operation.deleteKey, length, '', null);
    } else if (map.containsKey(Operation.retainKey)) {
      final length = map[Operation.retainKey] as int?;
      return Operation._(Operation.retainKey, length, '',
          map[Operation.attributesKey] as Map<String, dynamic>?);
    }
...
class Delta {
  ...
  static Map<String, dynamic> invertAttributes(
    ...
    final baseInverted = base.keys.fold({}, (memo, key) {
