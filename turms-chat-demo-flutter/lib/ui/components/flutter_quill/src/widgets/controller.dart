...
class QuillController extends ChangeNotifier {
  ...
  void _indentSelectionFormat(bool isIncrease) {
    ...
    final value = indent.value as int;
    if (value == 1 && !isIncrease) {
      formatSelection(Attribute.clone(Attribute.indentL1, null));
      return;
    }
    if (isIncrease) {
      if (value < 5) {
        formatSelection(Attribute.getIndentLevel(value + 1));
      }
      return;
    }
    formatSelection(Attribute.getIndentLevel(value - 1));
  ...
  void _indentSelectionEachLine(bool isIncrease) {
    ...
    for (final style in styles) {
      ...
      } else if (value == 1 && !isIncrease) {
        formatAttribute = Attribute.clone(Attribute.indentL1, null);
      } else if (isIncrease) {
        if (value < 5) {
          formatAttribute = Attribute.getIndentLevel(value + 1);
        }
      } else {
        formatAttribute = Attribute.getIndentLevel(value - 1);
      }
