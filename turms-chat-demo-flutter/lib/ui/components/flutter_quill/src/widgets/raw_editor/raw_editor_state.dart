...
class QuillRawEditorState extends EditorState
  ...
  List<ContextMenuButtonItem> get contextMenuButtonItems =>
      EditableText.getEditableButtonItems(
        ...
        onCopy: copyEnabled && !controller.selection.isCollapsed
            ? () => copySelection(SelectionChangedCause.toolbar)
            : null,
        onCut: cutEnabled && !controller.selection.isCollapsed
  ...
  VerticalSpacing _getVerticalSpacingForLine(
    ...
    if (attrs.containsKey(Attribute.header.key)) {
      ...
      if (attrs[Attribute.header.key]!.value is double) {
        level = (attrs[Attribute.header.key]!.value as double).toInt();
      } else {
        level = attrs[Attribute.header.key]!.value as int;
      }
      switch (level) {
        case 1:
  ...
  bool _shouldShowSelectionHandles() =>
      widget.configurations.showSelectionHandles &&
  ...
  Future<LinkMenuAction> _linkActionPicker(Node linkNode) async {
    final link =
        linkNode.style.attributes[Attribute.link.key]!.value! as String;
  ...
  void requestKeyboard() {
    ...
    if (_hasFocus) {
      ...
      openConnectionIfNeeded();
      if (!keyboardAlreadyShown && isMobile(supportWeb: false)) {
  ...
  Future<void> pasteText(SelectionChangedCause cause) async {
    ...
    if (onImagePaste != null) {
      final image = await _readImageFromClipboard();
      if (image == null) {
        return;
      }
      ...
      final imageBytes = await image.readAll();
      // TODO: optimize
      final base64Image = base64Encode(imageBytes);
      controller.replaceText(
        textEditingValue.selection.end,
        0,
        BlockEmbed.image(base64Image),
        null,
      );
    }
  }

  Future<DataReaderFile?> _readImageFromClipboard() async {
    final reader = await ClipboardReader.readClipboard();
    final completer = Completer<DataReaderFile>();
    if (reader.canProvide(Formats.png)) {
      reader.getFile(Formats.png, completer.complete,
          onError: completer.completeError);
    } else if (reader.canProvide(Formats.jpeg)) {
      reader.getFile(Formats.jpeg, completer.complete,
          onError: completer.completeError);
    } else if (reader.canProvide(Formats.gif)) {
      reader.getFile(Formats.gif, completer.complete,
          onError: completer.completeError);
    } else {
      return Future.value();
    }
