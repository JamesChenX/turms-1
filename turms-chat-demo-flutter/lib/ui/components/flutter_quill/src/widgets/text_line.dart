...
class _TextLineState extends State<TextLine> {
    ...
    for (var child in widget.line.children) {
      if (child is Embed) {
        ...
        if (child.value.type == BlockEmbed.customType) {
          child =
              Embed(CustomBlockEmbed.fromJsonString(child.value.data as String))
  ...
  TextStyle _getInlineTextStyle(
      ...
      if (nodeStyle.values.any((v) => v.key == k)) {
        if (k == Attribute.underline.key || k == Attribute.strikeThrough.key) {
          ...
          if (color?.value is String) {
            textColor = stringToColor(color?.value as String?, textColor);
          }
    ...
    if (font != null && font.value != null) {
      res = res.merge(TextStyle(fontFamily: font.value as String?));
    }
    ...
    if (size != null && size.value != null) {
      switch (size.value) {
        ...
        default:
          res = res
              .merge(TextStyle(fontSize: getFontSize(size.value) as double?));
      }
    }
    ...
    if (color != null && color.value != null) {
      var textColor = defaultStyles.color;
      if (color.value is String) {
        textColor = stringToColor(color.value as String);
      }
      if (textColor != null) {
    ...
    if (background != null && background.value != null) {
      final backgroundColor = stringToColor(background.value as String?);
  ...
  void _tapNodeLink(Node node) {
    final link = node.style.attributes[Attribute.link.key]!.value as String?;
