...
pub fn resize(input_path: String, output_path: String, width: u32, height: u32) -> ResizeResult {
...
fn resize0(input_path: String, output_path: String, width: u32, height: u32) -> ImageResult<bool> {
...
fn resize_gif(input_path: &String, output_path: &String, width: u32, height: u32) -> ImageResult<bool> {
    match File::open(input_path) {
        Ok(f) => {
            ...
            if let Ok(frames) = raw_frames.collect_frames() {
                ...
                for frame in &frames {
                    let resized =
                        imageops::resize(&frame.buffer().clone(), width, height, FilterType::CatmullRom);
                ...
                match File::create(output_path) {
                    Ok(gif_out) => {
                        ...
                        encoder.encode_frames(resized_frames.into_iter())?;
                        Ok(true)
                    }
                    Err(e) => {
