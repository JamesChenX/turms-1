...
import im.turms.service.domain.admin.repository.AdminUserRepository;
...
public class AdminUserService extends BaseAdminService {
    ...
    public AdminUserService(
        ...
        super(passwordManager, adminUserRepository, adminRoleService);
    ...
    public Flux<Long> queryRoleIds(@NotEmpty Set<String> accounts) {
        ...
        for (String account : accounts) {
            ...
            if (adminInfo != null) {
                roleIds.add(adminInfo.getAdminUser()
                        .getRoleId());
            }
        }
        return roleIds.size() == accounts.size()
                ? Flux.fromIterable(roleIds)
                : queryAdmins(accounts, null, null, null).map(AdminUser::getRoleId);
    }

    public Mono<AdminUser> authAndAddAdmin(
    ...
    public Mono<AdminUser> addAdmin(
            @Nullable @NoWhitespace @Size(
                    min = MIN_ACCOUNT_LIMIT,
                    max = MAX_ACCOUNT_LIMIT) String account,
            @Nullable @NoWhitespace @Size(
        ...
        registrationDate = registrationDate == null
                ? new Date()
                : registrationDate;
        AdminUser adminUser = new AdminUser(account, password, name, roleId, registrationDate);
        AdminInfo adminInfo = new AdminInfo(adminUser, rawPassword);
        String finalAccount = account;
        Mono<Void> result = upsert
                ? adminUserRepository.upsert(adminUser)
                        .then()
    ...
    public Flux<AdminUser> queryAdmins(
            @Nullable Set<String> accounts,
            @Nullable Set<Long> roleIds,
            @Nullable Integer page,
            @Nullable Integer size) {
        return adminUserRepository.findAdminUsers(accounts, roleIds, page, size);
    }

    public Mono<DeleteResult> authAndDeleteAdminUsers(
            @NotNull String requesterAccount,
    ...
    private Mono<DeleteResult> deleteAdmins(@NotEmpty Set<String> accounts) {
        ...
        return adminUserRepository.deleteByIds(accounts)
                .map(result -> {
    ...
    public Mono<UpdateResult> updateAdmins(
        ...
        byte[] password = rawPassword == null
                ? null
                : passwordManager.encodeAdminPassword(rawPassword);
        return adminUserRepository.updateAdminUsers(targetAccounts, password, name, roleId)
    ...
    public Mono<Long> countAdmins(@Nullable Set<String> accounts, @Nullable Set<Long> roleIds) {
        return adminUserRepository.countAdminUsers(accounts, roleIds);
    }
